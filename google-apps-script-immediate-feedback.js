function onOpen() {
  try {
    ...
    ui.createMenu('Business Tools')
      ...
      .addItem('Test Immediate Updates', 'testImmediateUpdates')
      .addToUi();
  } catch (error) {
...
function testImmediateUpdates() {
  try {
    var sheet = SpreadsheetApp.getActiveSheet();
    
    // Test 1: Update cell A1
    sheet.getRange(1, 1).setValue("Test 1: " + new Date().toLocaleTimeString());
    SpreadsheetApp.flush();
    Utilities.sleep(2000);
    
    // Test 2: Update cell A2
    sheet.getRange(1, 2).setValue("Test 2: " + new Date().toLocaleTimeString());
    SpreadsheetApp.flush();
    Utilities.sleep(2000);
    
    // Test 3: Update cell A3
    sheet.getRange(1, 3).setValue("Test 3: " + new Date().toLocaleTimeString());
    SpreadsheetApp.flush();
    Utilities.sleep(2000);
    
    Browser.msgBox('Test completed! Check cells A1, A2, A3 for timestamps.');
  } catch (error) {
...
function removeUnnecessaryColumns() {
  ...
  try {
    ...
    // More comprehensive list of columns to remove
    var columnsToRemove = [
      'Category',
      'Review Rating', 
      'Number of Reviews',
    ...
    for (var i = 0; i < columnsToDelete.length; i++) {
      ...
      // Force refresh after each deletion
      SpreadsheetApp.flush();
      Utilities.sleep(1000);
    }
