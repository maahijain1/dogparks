...
type ListingWithRelations = Listing & { cities?: City & { states?: State } }

interface HomeClientProps {
  initialStates?: State[]
  initialCities?: CityWithState[]
  initialListings?: ListingWithRelations[]
  initialFeaturedListings?: ListingWithRelations[]
...
export default function HomeClient({
  ...
  const [cities, setCities] = useState<CityWithState[]>(initialCities)
  const [states, setStates] = useState<State[]>(initialStates)
  const [featuredListings, setFeaturedListings] = useState<ListingWithRelations[]>(initialFeaturedListings)
  const [allListings, setAllListings] = useState<ListingWithRelations[]>(initialListings)
  ...
  const [itemsPerPage] = useState(12) // Reduced from 30 to 12 for better performance
  ...
  const [dynamicSettings, setDynamicSettings] = useState(initialSettings)
                  ...
                  {Array.from({ length: Math.min(5, articlesTotalPages) }, (_, i) => {
                    ...
                    return (
                      <button
                        ...
                        className={`px-3 py-2 text-sm font-medium border ${articlesCurrentPage === pageNum
