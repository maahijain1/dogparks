/**
 * Robust Business Script with Error Handling
...
function debugScript() {
  try {
    ...
    debugInfo += 'Sheet Name: ' + sheet.getName() + '\n';
    debugInfo += 'Total Rows: ' + data.length + '\n';
    debugInfo += 'Total Columns: ' + headers.length + '\n\n';
    ...
    debugInfo += 'Column Indices:\n';
    debugInfo += 'Business Name: ' + businessNameCol + '\n';
    debugInfo += 'Email: ' + emailCol + '\n';
    debugInfo += 'City Link: ' + cityLinkCol + '\n\n';
    
    // Check Gmail drafts
    try {
      var drafts = GmailApp.getDrafts();
      debugInfo += 'Gmail Drafts: ' + drafts.length + ' found\n';
    } catch (e) {
      debugInfo += 'Gmail Error: ' + e.toString() + '\n';
    }
...
function generateCityLinks() {
  try {
    ...
    SpreadsheetApp.getUi().alert("❌ Error in generateCityLinks: " + error.toString());
  }
}
  ...
  try {
    ...
    if (businessNameCol === -1 || emailCol === -1 || cityLinkCol === -1) {
      SpreadsheetApp.getUi().alert('❌ Error: Required columns not found!\n\n' +
        'Business Name: ' + (businessNameCol !== -1 ? 'Found' : 'Missing') + '\n' +
        'Email: ' + (emailCol !== -1 ? 'Found' : 'Missing') + '\n' +
        'City Link: ' + (cityLinkCol !== -1 ? 'Found' : 'Missing'));
      return;
    }
    ...
    addProgressTrackingRow(sheet, emailSentCol);
    ...
    for (var i = 1; i < data.length; i++) {
      try {
        ...
        try {
          ...
          // Toast might fail, continue anyway
        }
    ...
    SpreadsheetApp.getUi().alert('❌ Critical Error in sendEmailCampaign: ' + error.toString());
  }
}

// Progress tracking functions
function addProgressTrackingRow(sheet, emailSentCol) {
  try {
    // Insert a new row after the header for progress tracking
    sheet.insertRowAfter(1);
    
    // Add progress labels
    sheet.getRange(2, 1).setValue("📊 PROGRESS TRACKER");
    sheet.getRange(2, 1).setFontWeight("bold");
    sheet.getRange(2, 1).setBackground("#e3f2fd");
    
    // Add progress status cell
    sheet.getRange(2, emailSentCol).setValue("⏳ Ready to start...");
    sheet.getRange(2, emailSentCol).setBackground("#fff3cd");
    sheet.getRange(2, emailSentCol).setFontWeight("bold");
  } catch (error) {
    // If progress tracking fails, continue without it
  }
}
...
function updateProgressDisplay(sheet, current, total, status) {
  try {
    ...
    // If progress display fails, continue without it
  }
}
...
function checkEmailStatus() {
  try {
    ...
    SpreadsheetApp.getUi().alert('❌ Error in checkEmailStatus: ' + error.toString());
  }
}
...
function removeUnnecessaryColumns() {
  try {
    ...
    SpreadsheetApp.getUi().alert('❌ Error in removeUnnecessaryColumns: ' + error.toString());
  }
}
...
function getEmailTemplate() {
  try {
    ...
    SpreadsheetApp.getUi().alert('❌ Error getting email template: ' + error.toString());
    return null;
  }
}
